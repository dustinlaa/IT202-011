...
LEFT JOIN (SELECT * FROM CompetitionParticipants WHERE user_id = :uid) as cp ON cp.comp_id = Competitions.id WHERE expires > current_timestamp() AND paid_out < 1 ORDER BY expires desc");
